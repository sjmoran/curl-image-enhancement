...
def _glob_net(layer, in_channels):
    """The global branch of an MSCA block: context pooled to a 64-vector."""
    return nn.Sequential(
        layer(in_channels, 64, 3, 2, 1),
...
class MSCA(nn.Module):
    """One multi-scale contextual awareness skip connection.

    The dilation-2 branch, the dilation-4 branch and the global branch of
    Figure 3, concatenated with the encoder features they came from and fused
    back to the encoder's own channel count by a 1x1 convolution.
    """

    def __init__(self, channels, layer):
        super().__init__()
        self.mid_net2 = MidNet2(in_channels=channels)
        self.mid_net4 = MidNet4(in_channels=channels)
        self.glob_net = _glob_net(layer, channels)
        self.conv_fuse = nn.Conv2d(channels + 64 * 3, channels, 1)

    def forward(self, feat):
        mid2 = self.mid_net2(feat)
        mid4 = self.mid_net4(feat)
        glob = self.glob_net(feat)[:, :, None, None]
        glob = glob.repeat(1, 1, mid2.shape[2], mid2.shape[3])
        return self.conv_fuse(torch.cat((feat, mid2, mid4, glob), 1))
...
class TED(nn.Module):
    ...
    def __init__(self, msca_every_level: bool = False):
        ...
        :param msca_every_level: the paper's parameter-heavier variant, with an
            MSCA-skip connection at every encoder level rather than only the
            first. Section IV-A uses it for the ablation tables; the
            parameter-light default is what Section IV-B compares against the
            state of the art.
        :returns: N/A
        :rtype: N/A
        ...
        self.msca_every_level = msca_every_level
        ...
        # With a skip at every level the decoder receives the fused encoder
        # features alongside its own, so each stage takes twice the channels.
        wide = 2 if msca_every_level else 1
        self.dconv_up4 = LocalNet(128 * wide, 64)
        self.dconv_up3 = LocalNet(64 * wide, 32)
        self.dconv_up2 = LocalNet(32 * wide, 16)
        self.dconv_up1 = LocalNet(32, 3)

        self.conv_fuse1 = nn.Conv2d(208, 16, 1)

        if msca_every_level:
            self.msca2 = MSCA(32, layer)
            self.msca3 = MSCA(64, layer)
            self.msca4 = MSCA(128, layer)
    ...
    def forward(self, x):
        ...
        if self.msca_every_level:
            x = torch.cat([x, self.msca4(conv4)], dim=1)
        ...
        if self.msca_every_level:
            x = torch.cat([x, self.msca3(conv3)], dim=1)
        ...
        if self.msca_every_level:
            x = torch.cat([x, self.msca2(conv2)], dim=1)
...
class TEDModel(nn.Module):
    ...
    def __init__(self, msca_every_level: bool = False):
        ...
        :param msca_every_level: an MSCA-skip connection at every encoder
            level rather than only the first
        :returns: N/A
        :rtype: N/A
        ...
        self.ted = TED(msca_every_level=msca_every_level)
